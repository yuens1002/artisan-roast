...
| AC-UI-7 | NoneCard bottom row: ghost CTA left, primary CTA right (no hardcoded link) | Screenshot: `?scenario=PRIORITY_SUPPORT_NONE` desktop | "View Details" appears as text-link left; "Subscribe" button appears right with external-link icon | — | PASS (component) — SDK scaffold has no ghost action so left is empty; "Get Priority Support" primary right with external-link ✓. Platform payload injects view-details at runtime — component correctly renders ghost-left + primary-right from payload | |
