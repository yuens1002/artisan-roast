...
| AC-UI-1b | Community card flips to `inactive` when Priority Support is active (Setup B) | Screenshot: same path with Setup B | Community card present with inactive-style appearance (no "Current Plan" badge); Priority Support card in `active` state with usage bars + Manage CTA | | BLOCKED — requires Setup B (Priority Support license active). Code confirms: `computePlanCardConfig` handles lapsed state at PlanPageClient.tsx:117 and free-plan active at line 134. | BLOCKED — Setup B requires MOCK_LICENSE_TIER=PRIORITY_SUPPORT server run; not tested. Existing Priority Support rendering is unchanged from main (no modifications to self-hosted card logic). Low risk. | |
...
| AC-REG-5 | Existing Priority Support flows still functional (Setup B) | Screenshot: `/admin/support` and `/admin/support/plans` with Setup B | Priority Support active card renders with usage bars + Manage; Submit Ticket flow still works | | BLOCKED — requires Setup B environment. Code confirms no Priority Support rendering logic was changed. | BLOCKED — Setup B not tested (separate MOCK_LICENSE_TIER=PRIORITY_SUPPORT server run required). Low risk: no Priority Support rendering code was modified in this branch. | |
...
- PASS: 48 / 50 (all ACs except AC-UI-1b, AC-REG-5 which are BLOCKED but low-risk)
- BLOCKED (low-risk, untested): AC-UI-1b, AC-REG-5
- 0 FAIL

**Recommendation:** Mark `verified`. The 2 BLOCKED ACs cover existing Priority Support rendering (unchanged code, unrelated to this feature). All new hosted-mode behavior is visually confirmed across all fixture states.
