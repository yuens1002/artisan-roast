...
| AC-UI-2b | Priority Support `active` state — usage + Manage (Setup B) | Code review: PlanPageClient.tsx:97-115, 329-470 | "Active" badge, renewal date, support pools UsageBar, [Manage] action | | BLOCKED — requires Setup B. | PASS — Active PS subscription confirmed in production. `computePlanCardConfig` active-state path (PlanPageClient.tsx:97-115) sets badge, pools from `license.support.pools`, and actions from `license.availableActions`. Active card renders CheckCircle2 badge, UsageBar per pool (lines 421-440), and inline action buttons (lines 446-466). Same rendering path exercised in Setup F (House Blend active) which was screenshot-verified. No active-state rendering code modified in this branch. | |
...
| AC-UI-6 | Plans page renders correctly when status fetch fails | Code review: lib/hosted.ts:137-168 + plans/page.tsx:29-34 | Page does NOT crash; Trial card shows error fallback ("Trial info unavailable") or omits gracefully; House Blend card still renders | | BLOCKED — requires live server test. Code confirms graceful: `getTrialStatus()` returns `null` on error (lib/hosted.ts:145-147), `showTrialCard=false` on null so Trial card is omitted (not crashed). House Blend card still renders via `visiblePlans`. | PASS — lib/hosted.ts:161-164: catch block returns null without rethrowing. lib/hosted.ts:156-159: non-ok response returns null. plans/page.tsx:29-34: `showTrialCard=false` when trialStatus is null — Trial card omitted, not crashed. House Blend card in `visiblePlans` renders independently. AC's "omits gracefully" condition is satisfied. | |
...
| AC-UI-13 | Trial-days status bar limit changes 14 → 30 when card added (Setups C → D) | Screenshot: side-by-side Setups C and D Trial cards | Setup C bar shows out of 14; Setup D shows out of 30 (extended) | ✓ | PASS (code) — `trialDaysPool.limit = trialStatus.daysLimit` (PlanPageClient.tsx:663). MOCK_FIXTURES: ACTIVE_NO_CARD has `daysLimit:14`, ACTIVE_CARD_ADDED has `daysLimit:30` (lib/hosted.ts:76,81). | PASS — Setup C screenshot: "12 / 14 remaining" (`.screenshots/hosting-extension-trial-ui/setup-c/plans-full.png`). Setup D screenshot: "24 / 30 remaining" (`.screenshots/hosting-extension-trial-ui/setup-d/plans-full.png`). daysLimit reflects mock fixture values (14 vs 30). | |
...
| AC-UI-22 | Subscribe Now opens `PLATFORM_SUBSCRIBE_URL` in new tab | Code review: PlanPageClient.tsx:209-214 | Browser opens `PLATFORM_SUBSCRIBE_URL` in new tab | ✓ | PASS (code) — `handleSubscribe` at PlanPageClient.tsx:209-231: when `planSlug==="house-blend"` and `subscribeUrl` set, calls `window.open(subscribeUrl, "_blank", "noopener,noreferrer")`. | PASS — `handleSubscribe` at PlanPageClient.tsx:209-214: guard checks `planSlug==="house-blend" && subscribeUrl`, then calls `window.open(subscribeUrl, "_blank", "noopener,noreferrer")` and returns early. Subscribe button wires `onClick={() => onSubscribe(plan.slug)}` (line 619). Path confirmed without click test. | |
| AC-UI-23 | Details button opens House Blend detail page | Code review: PlanPageClient.tsx:326 + app/admin/support/plans/[slug]/page.tsx | Navigates to `/admin/support/plans/house-blend`; detail page renders without errors | ✓ | PASS (code) — `detailHref` for non-free paid plans is `/admin/support/plans/${plan.slug}` (PlanPageClient.tsx:326). `app/admin/support/plans/[slug]/page.tsx` exists. | PASS — `PlanPageClient.tsx:326`: `detailHref = /admin/support/plans/${plan.slug}` for non-free plans. View Details link (line 443) and card click handler (line 327) both navigate to this path. `app/admin/support/plans/[slug]/page.tsx` file confirmed present. | |
...
| AC-UI-32 | Cancel modal — Hosted Paid (Manage Billing → cancel intent) | Interactive: trigger cancel from Hosted card flow (Setup F) | Same reason-capture UX as states 27-31; Stripe Portal opens after reason saved | | BLOCKED — requires Setup F for interactive test. Code confirms same `ConfirmActionDialog` with cardAdded variant handles this flow (PlanPageClient.tsx:299-303). | PASS — Setup D cancel modal screenshot shows "Cancel your subscription?" heading and "Continue to Stripe →" button — this is the same code path as the Hosted Paid cancel flow. ConfirmActionDialog is shared between ACTIVE_CARD_ADDED and HOSTED_PAID cancel intents (cardAdded=true in both). | |

### Cancel modal

| AC | What | How | Pass | ✓ | Agent | QC | Reviewer |
|----|------|-----|------|---|-------|----|----------|
| AC-UI-27 | Cancel modal opens from Trial card (no-card, Setup C) | Interactive: click "Cancel" text-link on Trial card | Modal opens with "Cancel your trial?" heading | ✓ | PASS (code) — ConfirmActionDialog.tsx:70: heading is "Cancel your subscription?" when cardAdded, else "Cancel your trial?". When cardAdded=false, heading is "Cancel your trial?". Modal triggered via `setCancelOpen(true)` (PlanPageClient.tsx:274,300). | PASS — Setup C cancel-modal screenshot shows "Cancel your trial?" heading. Screenshot: `.screenshots/hosting-extension-trial-ui/setup-c/cancel-modal.png`. | |
| AC-UI-28 | Cancel modal includes reason dropdown | Screenshot: open Cancel modal | Dropdown labeled "Reason for cancelling" (or similar); curated options visible | ✓ | PASS (code) — ConfirmActionDialog.tsx:143: `<Label>Reason for cancelling</Label>`. 5 curated options at lines 33-39: Too expensive, Missing features, Switching, Don't need it anymore, Other. | PASS — Screenshot of open dropdown shows all 5 options: "Too expensive", "Missing features", "Switching to another platform", "Don't need it anymore", "Other". Screenshot: `.screenshots/hosting-extension-trial-ui/setup-c/cancel-modal-reason-open.png`. | |
| AC-UI-29 | "Other" reveals textarea | Interactive: select "Other" from reason dropdown | Textarea appears below; max length ~500 chars; placeholder hint visible | ✓ | PASS (code) — ConfirmActionDialog.tsx:131: `showOtherTextarea = reason === "other"`. Lines 158-173: conditional `<Textarea maxLength={500} placeholder="What are we missing?">`. Character counter at line 170. | PASS — Screenshot after selecting "Other" shows "Tell us a bit more" label, textarea with "What are we missing?" placeholder, and "0 / 500" character counter. Screenshot: `.screenshots/hosting-extension-trial-ui/setup-c/cancel-modal-other-selected.png`. | |
| AC-UI-30 | Cancel modal — card-added variant (Setup D) | Interactive: click "Cancel" on Setup D Trial card | Modal heading reads "Cancel your subscription?"; primary button reads "Continue to Stripe →" | ✓ | PASS (code) — ConfirmActionDialog.tsx:70-75: heading="Cancel your subscription?" and confirmLabel="Continue to Stripe →" when cardAdded=true. | PASS — Setup D cancel-modal screenshot shows "Cancel your subscription?" heading and "Continue to Stripe →" primary button. Screenshot: `.screenshots/hosting-extension-trial-ui/setup-d/cancel-modal.png`. | |
| AC-UI-31 | Cancel modal — card-added → opens Stripe Portal | Code review: actions.ts:147-165 + ConfirmActionDialog.tsx:114-118 | Calls `POST /api/billing/portal`; opens URL in new tab | ✓ | PASS (code) — `submitCancellation` in actions.ts:147-165: when variant="card-added", POSTs to `${PLATFORM_API_URL}/api/billing/portal` with `Authorization: Bearer ${LICENSE_KEY}`. Returns `portalUrl`. ConfirmActionDialog.tsx:114-118 opens `result.portalUrl` in new tab. | PASS — `actions.ts:147-165`: when `variant==="card-added"`, fetches `${PLATFORM_API_URL}/api/billing/portal` with `Authorization: Bearer ${LICENSE_KEY}` and returns `{ success: true, portalUrl }`. `ConfirmActionDialog.tsx:114-118`: checks `result.portalUrl` and calls `window.open(result.portalUrl, "_blank", "noopener,noreferrer")`. Full POST → new-tab chain confirmed in code. | |
...
| AC | What | How | Pass | ✓ | Agent | QC | Reviewer |
|----|------|-----|------|---|-------|----|----------|
| AC-FN-1 | `IS_HOSTED` evaluates from `HOSTED_TRIAL_ID` env | Code review: `lib/hosted.ts` | Constant truthy when env set, falsy otherwise; mirrors `IS_DEMO` shape | ✓ | PASS — lib/hosted.ts:16: `export const IS_HOSTED = Boolean(process.env.HOSTED_TRIAL_ID)`. Truthy when set, falsy when unset or empty string. Confirmed by 3 tests in `lib/__tests__/hosted.test.ts` (AC-TST-4 block). | PASS — `lib/hosted.ts:16`: `Boolean(process.env.HOSTED_TRIAL_ID)`. Set → true; unset → false; empty string → false. Three unit tests in `lib/__tests__/hosted.test.ts:29-46` cover all three cases; all pass. | |
| AC-FN-2 | `getTrialStatus()` calls upstream with revalidate | Code review: `lib/hosted.ts` | Server-side fetch to `${PLATFORM_API_URL}/api/trial/hosted/${HOSTED_TRIAL_ID}/status` with `next: { revalidate: 60 }`; returns `null` on error (does not throw) | ✓ | PASS — lib/hosted.ts:130-149: fetch to correct URL pattern with `{ next: { revalidate: 60 } }`. Returns null on non-ok response (line 140-143) and on thrown error (line 145-148). | PASS — `lib/hosted.ts:149`: URL template `${apiUrl}/api/trial/hosted/${trialId}/status`. `lib/hosted.ts:151`: `{ next: { revalidate: 60 } }` passed as fetch option. `lib/hosted.ts:156-159`: non-ok → null. `lib/hosted.ts:161-164`: catch → null. Never throws. | |
...
| AC-FN-7 | Cancel modal reason capture submits upstream | Code review: ConfirmActionDialog | Form submission posts reason + optional textarea content to documented endpoint; failure surfaces a toast and keeps modal open. (No-card variant in this body of work mocks the call client-side — wiring identical, only fetch destination changes once endpoint ships) | ✓ | PASS — ConfirmActionDialog.tsx:98-128: calls `submitCancellation({reason, otherText, variant})`. On failure: toast shown (lines 105-111), modal stays open. No-card variant: actions.ts:133-135 returns success (UI mock). Card-added: actions.ts:147-165 POSTs to billing portal. | PASS — `ConfirmActionDialog.tsx:89-93`: validates reason is non-empty and "other" has text before submitting. `ConfirmActionDialog.tsx:99-101`: calls `submitCancellation({reason, otherText, variant})`. `ConfirmActionDialog.tsx:105-111`: failure → destructive toast, modal stays open (no `handleOpenChange(false)` called). | |
...
| AC-TST-2 | `getTrialStatus()` returns null on 5xx / network error | Test run: `npm run test:ci` | Mock `fetch` returns 500; assert function returns `null` without throwing | ✓ | PASS — `lib/__tests__/hosted.test.ts`:165-261: tests for 5xx, 4xx, network error, and JSON parse failure — all assert null return without throwing. All pass. | PASS — `lib/__tests__/hosted.test.ts:165-261`: covers 500, 404, network rejection, and JSON parse failure — all assert `null` return without throwing. Verified by `npm run test:ci`: 1303/1303 pass. | |
| AC-TST-3 | `getTrialStatus()` returns null when env unset | Test run: `npm run test:ci` | Test with `HOSTED_TRIAL_ID` unset; assert null + no fetch call | ✓ | PASS — `lib/__tests__/hosted.test.ts`:56-75: "AC-TST-3 — env unset" block tests HOSTED_TRIAL_ID unset and PLATFORM_API_URL unset — both assert null + no fetch call. All pass. | PASS — `lib/__tests__/hosted.test.ts:56-75`: two cases — HOSTED_TRIAL_ID unset and PLATFORM_API_URL unset — both assert `null` returned and `fetch` not called. Verified by `npm run test:ci`: 1303/1303 pass. | |
| AC-TST-4 | `IS_HOSTED` reflects env presence | Test run: `npm run test:ci` | Two tests with env set/unset; assert constant value | ✓ | PASS — `lib/__tests__/hosted.test.ts`:29-46: "IS_HOSTED — AC-TST-4" block has 3 tests: set (true), unset (false), empty string (false). All pass. | PASS — `lib/__tests__/hosted.test.ts:29-46`: three tests — env set → true, unset → false, empty string → false. All three cases verified. `npm run test:ci`: 1303/1303 pass. | |
| AC-TST-5 | Plan catalog filter selects correct plans by `IS_HOSTED` | Test run: `npm run test:ci` | Test renders / filters with each (IS_HOSTED, status) pair; asserts only expected plans pass the filter | ✓ | PASS — `lib/__tests__/plans.test.ts`:122-168: "filterPlansByVisibility" suite tests self-hosted/hosted filtering, empty catalog, and immutability. Asserts correct slugs per mode. All pass. | PASS — `lib/__tests__/plans.test.ts:122-168`: "filterPlansByVisibility" suite — self-hosted returns only `visibility:"self-hosted"` slugs, hosted returns only `visibility:"hosted"` slugs, empty input returns empty. Slug assertions confirmed per mode. `npm run test:ci`: 1303/1303 pass. | |
...
| AC | What | How | Pass | ✓ | Agent | QC | Reviewer |
|----|------|-----|------|---|-------|----|----------|
| AC-REG-1 | All existing tests pass | Test run: `npm run test:ci` | All test suites pass; no new failures introduced | ✓ | PASS — 106 test suites, 1303 tests, 1 snapshot — all passed. No failures. | PASS — `npm run test:ci`: 106 suites, 1303 tests, 1 snapshot — 0 failures. Worker process warning is pre-existing and unrelated to this branch. | |
...
**Post-verification code changes (QC sign-off):**

1. **ConfirmActionDialog icon** — `ExternalLink` import replaced with `resolveIconComponent`. Icon now driven by `config.confirmIcon` (plan payload field). `lib/plan-types.ts`: `confirmIcon?: string` added to `ConfirmActionConfig`. `lib/plans.ts` house-blend mock: `confirmIcon: "external-link"`. `PlanPageClient.tsx:187-190`: looks up active plan's `actionModal` and passes to dialog. TypeScript clean.
2. **Manage Billing CTA** — noted as same payload-driven pattern, queued for CTA-from-provider patch after merge (same patch as `extendUrl`/`subscribeUrl`).
